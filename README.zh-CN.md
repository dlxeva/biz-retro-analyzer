<p align="right">
  <a href="./README.md">English</a> | <a href="./README.zh-CN.md"><strong>简体中文</strong></a>
</p>

![Biz Retro Analyzer banner](./assets/banner.svg)

# Biz Retro Analyzer / 会后复盘分析 Skill

把复杂协作型项目中的杂乱材料，转成有证据支撑的分析资产。

`biz-retro-analyzer` 是一个可复用的智能体 skill，适合分析：

- 会议录音逐字稿
- 会议纪要
- 现场观察记录
- 视频分析文本
- 可选的后续分析对话

并将其转化为：

- 结构化事实底稿
- 项目/事项演进脉络
- 角色诉求分析
- 与会方对议题的理解层级分析
- 协同结构与推进结构分析
- 关键判断与反向审计
- 下一步动作
- 可复用的方法论笔记

它适用于对话密集、材料混杂、需要持续判断的复杂协作型项目。重点不是只做摘要，而是回答：

- 方向是怎么变化的
- 各方真正想要什么
- 各方到底懂这个议题到什么程度
- 这个项目实际上是如何被推进、阻塞、改写或收敛的
- 当前哪些问题阻塞推进
- 哪些证据缺口现在可以后置，但后面必须补

---

## 这是什么

这**不是**转录工具，也**不是**通用会议纪要模板。

它是一个**证据优先（evidence-first）的分析 skill**，用于把原始对话材料变成可复用的“多方理解结构 + 推进结构”分析资产。

以下情况尤其适合使用：

- 多场会议并存
- 多种材料混在一起
- 事实与判断混杂
- 多方角色关系复杂
- 试点场景和方向持续变化
- 多方协同、谈判或推进摩擦明显

---

## 最适合的场景

适合用于：

- 会后复盘
- 合作推进回顾
- 项目演进分析
- 复杂项目推进结构分析
- 多方协作型复盘
- 现场观察与会议材料联合分析
- “到底发生了什么、什么变了、下一步该怎么办”这类问题

例如：

- 分析一次重要工作会后的多场逐字稿
- 把录音、纪要和现场观察整理成一条清晰脉络
- 审计当前叙事是否真的有证据支撑
- 区分试点场景和系统/产品本体

---

## 不适合的场景

以下场景**不建议**作为主工具使用：

- 普通会议纪要
- 简单 action items 提取
- 情绪分析
- 通用销售电话评分
- 纯转录
- 纯语气风格评估

如果你只需要“摘要 + 下一步动作”，这个 skill 可能过重。

---

## 核心设计

这个 skill 依赖四条原则：

1. **证据优先**  
   先保留原始材料，再做解释。

2. **事实先于判断**  
   先有结构化事实层，再有结论和判断层。

3. **角色真实诉求很重要**  
   区分“他说了什么”和“他真正想要什么”。

4. **理解层级也很重要**  
   不只看“他想要什么”，还看“他真正懂什么、懂到哪一层”。

5. **必要时强制反向审计**  
   如果叙事过于顺滑或自信，就反过来压力测试这些结论。

---

## 输入模型

这个 skill 支持分层输入。

### 第一层：原始证据

至少提供以下之一：

- 逐字稿
- 会议纪要
- 现场观察记录
- 视频分析文本

如果用户直接给的是音频，应该先转成可用逐字稿，再进入后续分析。

还应尽量识别互动上下文。可用标签包括：

- `formal_meeting`
- `working_session`
- `informal_debrief`
- `private_side_conversation`
- `asymmetric-awareness conversation`

这些上下文很重要，因为非正式或非对称知情材料往往更接近真实顾虑，但也更不稳定。它们适合用于识别顾虑、约束、推进阻力和协同风险，不应在缺少跨材料支持时直接上升为稳定意图、正式立场或承诺。

### 第二层：上下文补充（可选）

- 项目/事项背景
- 当前合作关系
- 用户自己的疑问
- 初步设想或假说

### 第三层：后续分析对话（可选）

- 用户与 AI 的分析对话
- 后续复盘讨论
- 中间结论
- 纠偏和修正过程

注意：后续分析对话只能作为**判断层补料**，不能当成原始事实。

### 非对称知情材料的伦理边界

这个 skill 可以分析非正式或非对称知情条件下的项目材料，但仅限于项目相关的：

- 理解分析
- 协同分析
- 风险分析
- 激励与约束分析
- 决策条件分析

不应用于产出：

- 人格画像
- 超出项目范围的个人利用策略
- 基于他人在非对称知情条件下表达而形成的操控性建议

这类材料导出的判断应保持收窄、内部使用、待验证的默认姿态。

---

## 输出模型

根据输入深度和需求，skill 会输出以下一种或多种结果：

1. **结构化事实底稿**
2. **复盘脉络 / 项目线程**
3. **关键判断与反向审计**
4. **当前动作建议**
5. **方法论沉淀**

目标是清晰区分：

- 哪些是已确认事实
- 哪些是推断
- 哪些证据还缺
- 下一步该做什么

### HTML 输出模式

这个 skill 现在也支持 `HTML` 报告输出模式，适合以下情况：

- 需要生成可直接分享的单页报告
- 需要比 Markdown 更强的视觉分层
- 需要把证据层、项目脉络、反向审计和动作建议放进同一页面

这套 HTML 输出模式由以下文件驱动：

- `references/html-output-mode.md`
- `references/output-schema.json`
- `assets/html-report-template.html`
- `assets/html-report.css`

用户不需要一开始就知道这个模式存在。  
当分析结果已经具备完整结构时，skill 可以在输出阶段主动提示：“这份内容已经适合升级为 HTML 报告页”。

### 证据标注

这个 skill 现在支持一套轻量的证据标注约定，用来标识重要判断的证据状态：

- `Status`：`Confirmed` / `Inferred` / `Assumption` / `Needs validation`
- `Strength`：`High` / `Medium` / `Low`
- `Basis`：`Source quote/paraphrase` / `Cross-source pattern` / `User context` / `Later reasoning only`
- `Source anchor`：对高影响判断补充简短来源锚点，如会议、材料、说话人或片段

它尤其适合标注以下内容：

- 角色诉求判断
- 项目边界判断
- 定价或杠杆判断
- 会影响承诺和推进方式的下一步动作

### 高风险判断闸门

当前技能对以下五类判断采用更严格的约束：

- 人物动机
- 客户真实意图
- 预算与报价策略判断
- 组织或政治站位判断
- 合作操盘权与实际控制关系判断

默认规则：

- 除非原始材料表达非常直接且结论保持足够窄，否则不要标成 `Confirmed`
- 如果只来自一次表达或一次会议，最高只写成 `Inferred`，强度为 `Low` 或 `Medium`
- 如果会影响报价、范围、推进节奏或承诺动作，必须同时写出 `Status`、`Strength`、`Basis`、`Source anchor` 和仍待验证事项
- 如果无法给出来源锚点，就降级或不输出

### Participant Understanding Map 默认要求

在 Mode B 和 Mode C 输出中，只要满足以下条件，就默认必须产出 `Participant Understanding Map`：

- 说话人归属可靠
- 参与者或角色之间存在实质差异

只有以下情况可以省略：

- 说话人归属不可靠
- 原始材料过薄，无法支撑人物级理解判断

如果省略，输出中必须显式说明原因。

这个 map 默认按以下四个维度来读参与者：

- 对项目问题本体的理解
- 对实际推进路径的理解
- 对执行与落地约束的理解
- 对决策与组织机制的理解

---

## 仓库结构

```text
biz-retro-analyzer/
├── SKILL.md
├── README.md
├── README.zh-CN.md
├── assets/
│   ├── banner.svg
│   ├── html-report-template.html
│   └── html-report.css
├── EVALUATION.md
├── LICENSE
├── agents/
│   └── openai.yaml
└── references/
    ├── html-output-mode.md
    ├── input-modes.md
    ├── output-schema.json
    ├── output-templates.md
    ├── analysis-checks.md
    ├── reverse-audit.md
    ├── interview-gap-prompts.md
    ├── failure-modes.md
    └── open-source-sanitization.md
```

### 文件职责

- `SKILL.md`：触发条件、工作流、硬规则、输出模式导航
- `assets/banner.svg`：仓库头图资产
- `assets/html-report-template.html`：可复用 HTML 报告骨架
- `assets/html-report.css`：可复用报告样式
- `references/html-output-mode.md`：何时使用 HTML 输出，以及如何组织页面
- `references/input-modes.md`：不同输入堆栈的处理方式
- `references/output-schema.json`：报告渲染前的结构化数据形状
- `references/output-templates.md`：可复用输出模板
- `references/analysis-checks.md`：质量与分析检查清单
- `references/reverse-audit.md`：反向审计流程
- `references/interview-gap-prompts.md`：下一轮补证据/补访谈的问题清单
- `references/failure-modes.md`：这个 skill 最容易漂移或判断失真的地方
- `references/open-source-sanitization.md`：如何让公开版本保持泛化和无敏感信息

---

## 快速开始

### 示例 Prompt

```text
使用 $biz-retro-analyzer 分析以下材料：

1. 三场会议逐字稿
2. 一份现场观察记录
3. 一段后续复盘分析对话

请输出：
- 每份材料的结构化事实底稿
- 一份跨材料脉络
- 角色诉求分析
- 一段反向审计
- 下一步动作清单
```

### 推荐工作流

1. 先只喂原始证据
2. 再补背景
3. 事实层稳定后，再补后续分析对话
4. 如果当前叙事太顺，做一次反向审计

---

## 迭代记录

最近的迭代变化见 [CHANGELOG.md](./CHANGELOG.md)。

当前这一轮重点包括：

- 新增默认的证据标注协议
- 收紧输出模板中的证据说明和待验证项
- 强化分析检查清单，对“判断是否有证据支撑”做显式复核
- 增加可见的迭代记录，方便后续公开维护

---

## 开源注意事项

这个仓库刻意保持通用表达。

不应该：

- 暴露真实客户名称
- 写死私有项目代号
- 在示例里保留可识别的个人/组织信息
- 依赖某一个行业背景才能理解

如果要新增例子，请先做脱敏和泛化。见 `references/open-source-sanitization.md`。

---

## 当前局限性

### 验证仍处于早期阶段

当前项目在协议设计、风险门控和输出结构上推进较快，但验证仍然偏早期。
现有测试更偏向协议合规和失败收敛，还没有形成覆盖更广真实材料和多模型行为的系统回归。

因此，当前版本更适合被理解为：

- 一套已经成形的分析协议
- 但验证仍在持续补强

### 输出效果依赖运行时执行保真度

这个仓库不是独立分析程序，而是 skill 协议加参考文档。
最终效果除了取决于底层模型，也取决于宿主 Agent Runtime 对 skill 结构的执行保真度。

主要兼容性风险不只是模型差异，还包括不同 runtime 对同一协议的行为漂移。

### 当前测试更偏协议检查，端到端验证仍需补足

现有测试样例刻意保持轻量，主要验证：

- 事实与判断分层
- 高风险判断收敛
- source / reasoning 分层
- `Participant Understanding Map` 强制产出
- informal / asymmetric-awareness 材料的安全处理

这符合当前阶段目标，但更完整的端到端验证仍需要：

- 更丰富的真实材料回归 case
- 多模型对比
- 对实际项目决策帮助程度的进一步观察

### 高风险判断闸门当前偏保守

目前对人物动机、真实意图、定价策略、组织站位等判断的门控是刻意偏保守的。
这有助于压住过度推断，但也可能让输出在某些快速商务场景里显得不够果断。

这个取舍在当前阶段是有意为之。
后续更可能需要按场景细化，而不是长期维持单一全局阈值。

### 可复用性当前仍偏向高经验用户

虽然这个 skill 已尽量保持通用化，但要用好它仍然依赖较强的分析经验。
使用者仍需要知道如何：

- 区分 source evidence 和 later reasoning
- 区分试探性表达和稳定意图
- 选择合适的输出 mode
- 解读 `Participant Understanding Map` 而不写过头

所以当前版本更接近：

- 面向高经验用户的分析协议

而不是：

- 面向普通用户的开箱即用产品

---

## 推荐扩展方向

这个 v0 是刻意轻量的。

适合放到 v1 的扩展包括：

- 行业模板
- 会型模板
- 基于脚本的格式化辅助
- taxonomy 与跨项目复用规范
- 仓库或知识库导出规范

---

## 评估

轻量评估方式见 `EVALUATION.md`。

---

## 许可协议

见 `LICENSE`。
